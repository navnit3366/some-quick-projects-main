...
class best_print:
    ...
    def print_progress2(self,
                        ...
                        newline: bool = False):
        i = int(i) + 1
        assert 1 <= int(i) <= n, "i<=n is not verified"
        M = min(int(length), int(n))
        k = M if (i % M == 0 or i == n) else i % M
        p = int(100 * k / M)
        self.print(
            f"{label}: step {math.ceil(i/M)}/{math.ceil(n/M)} [{'#'*k+' '*(M-k)}] {' '*(2-int(math.log10(p)))}({p}%) iter {i}/{n}\r",
            end="\n" if (newline or i == n) else "")
